...
@Table(name = "save_schedule")
@NamedQueries({
    @NamedQuery(
        name = "getAllSave_schedules",
        query = "SELECT ss FROM Save_schedule AS ss WHERE ss.schedule_date = :schedule_date ORDER BY ss.id DESC"
    ),
    @NamedQuery(
        name = "getAllSave_schedulesCount",
        query = "SELECT COUNT(ss) FROM Save_schedule AS ss WHERE ss.schedule_date = :schedule_date"
    ),
})
@Entity
public class Save_schedule {
    @Id
    @Column(name = "id")
